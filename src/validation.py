...
def backtest(dataset, as_of='2026-09-23', months=3, max_skus=30):
    ...
    for target in targets:
        ...
        for sku in skus:
            ...
            rows.append(dict(sku=sku,month=str(target.date()),actual=float(actual[sku]),model=prediction,baseline=naive))
    detail=pd.DataFrame(rows,columns=['sku','month','actual','model','baseline'])
    summary=[]
    if not detail.empty:
    ...
    scores=pd.DataFrame(summary,columns=['sku','months','actual_total','model_wape','baseline_wape'])
