...
orders_tab,detail_tab,anomaly_tab,backtest_tab,history_tab,quality_tab=st.tabs(['Рекомендации','Почему столько','Разовые продажи','Проверка прогноза','История','Качество данных'])
with orders_tab:
    ...
    with st.expander('Проверить и утвердить заказ'):
        ...
        if draft.empty:st.info('Нет положительных рекомендаций для утверждения.')
            ...
            reviewed=st.data_editor(draft.rename(columns=LABELS),hide_index=True,disabled=['Код 1С','Артикул','Товар','Поставщик','Рекомендация','Кратность','Минимальная партия'],column_config={'К заказу':st.column_config.NumberColumn(min_value=0)},key=f'approval-{run_signature[:16]}')
            ...
            if st.button('Утвердить и сохранить заказ',type='primary',disabled=approved.empty or not bad.empty or not acknowledge):
                try:
                    if st.session_state.get('saved_signature')!=run_signature:
                        st.session_state['saved_run']=store.save_run(ds.supplier,dataset_key,cfg,result,ds.sources)
                        st.session_state['saved_signature']=run_signature
                    selections=[{'sku':str(r['Код 1С']),'quantity':float(r['К заказу']),'reason':r['Причина корректировки']} for _,r in approved.iterrows()]
                    order_id=store.approve(st.session_state['saved_run'],selections,reviewer,order_note)
