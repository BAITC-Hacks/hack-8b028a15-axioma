...
def test_scenario_requires_acknowledgement_and_records_it(tmp_path):
    s,_,df=setup(tmp_path);df['stock_basis']='Сценарий: снимок без изменений'
    r=s.save_run('IEK','scenario',Settings(),df,[])
    with pytest.raises(ValueError):s.approve(r,[dict(sku='A',quantity=12)],'Manager')
    s.approve(r,[dict(sku='A',quantity=12)],'Manager',acknowledge_scenario=True)
    assert s.list_orders()[0]['rows'][0]['scenario_acknowledged'] is True


def test_large_fractional_quantity_cannot_pass_pack_validation(tmp_path):
    s,_,df=setup(tmp_path);df['pack']=1
    r=s.save_run('IEK','quantity',Settings(),df,[])
    with pytest.raises(ValueError):s.approve(r,[dict(sku='A',quantity=999999999999.5,reason='test')],'Manager')
