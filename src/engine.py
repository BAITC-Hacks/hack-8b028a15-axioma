...
def calculate(ds: Dataset, settings: Settings, _pooled=None):
    ...
    for _,p in ds.products.iterrows():
        ...
        if s.empty:
            results.append(dict(sku=sku,article=p.article,name=p['name'],category=str(p.category),supplier=ds.supplier,unit=p.unit,pack=max(number(p.pack,1),1),moq=max(number(p.moq,1),1),stock=p.stock,lead_days=sku_lead,expedite_need=np.nan,first_shortage=None,order_arrival=str((asof+pd.Timedelta(days=sku_lead)).date()),recommended=np.nan,status='Нет истории',reason='Нет завершённых месяцев продаж. Нужна ручная оценка.'));continue
        ...
        plan=inventory_plan(daily_path,receipts,stock,sku_lead,safety,pack,moq)
