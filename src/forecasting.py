...
def repeatable_annual_profile(rates,months):
    """Two full, active years with a material and similar calendar shape.

    Sparse demand does not justify a long selection window simply because a
    calendar year has elapsed. This gate is a planning heuristic, not a test of
    statistical significance.
    """
    profiles=[]
    for year in sorted({y for y,_ in months}):
        indexes=[i for i,(y,_) in enumerate(months) if y==year]
        if len(indexes)!=12:continue
        values=np.array([rates[i] for i in indexes])
        if np.count_nonzero(values)<9 or values.mean()<=0:continue
        profiles.append(values/values.mean())
    if len(profiles)<2:return False
    a,b=profiles[-2:]
    return min(float(a.std()),float(b.std()))>=.25 and float(np.corrcoef(a,b)[0,1])>=.8
...
def _fit_cached(values_tuple, months_tuple, use_seasonality, use_trend):
    ...
    # A six-month window can omit the very season we are about to forecast
    # (e.g. a September-only peak). With two years of data, validate a full cycle.
    window=12 if use_seasonality and repeatable_annual_profile(rates,months) else 6
    origins=list(range(max(6,len(rates)-window),len(rates)))
    scores={}; residuals={}
    ...
    return candidates,scores,residuals,len(origins)
...
def forecast_series(series, future_dates, method='auto', seasonality=True, trend=True, supplied=None):
    ...
    candidates,scores,residuals,n=_fit_cached(rates,months,seasonality,trend)
    ...
    band=float(np.quantile(err,.8)) if err else 0.
    monthly={}
    for dt in dates:
