...
class Store:
    ...
    def approve(self,run_id,selections,reviewer,note='',acknowledge_scenario=False):
        ...
        for choice in selections:
            ...
            base=catalog[sku]
            scenario=str(base.get('stock_basis','')).startswith('Сценарий:')
            if scenario and not acknowledge_scenario:raise ValueError('Подтвердите использование предполагаемых остатков для сценарного заказа')
            if base.get('recommended') is None or base.get('stock') is None:raise ValueError('Для товара не хватает данных')
            if isinstance(q,bool) or not isinstance(q,(int,float)) or not math.isfinite(q) or q<=0 or q>1e12:raise ValueError('Количество должно быть положительным конечным числом, не более 10¹²')
            moq=base.get('moq') or 1.;pack=base.get('pack') or 1.
            if q<moq or not math.isclose(q/pack,round(q/pack),rel_tol=0,abs_tol=1e-8):raise ValueError('Количество не соответствует минимуму или кратности')
            reason=str(choice.get('reason','')).strip()
            if not math.isclose(q,float(base['recommended']),rel_tol=0,abs_tol=1e-8) and not reason:
                raise ValueError('При изменении количества укажите причину')
            rows.append({**base,'approved_quantity':float(q),'adjustment_reason':reason,'scenario_acknowledged':bool(scenario and acknowledge_scenario)})
