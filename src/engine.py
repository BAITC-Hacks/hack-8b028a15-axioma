...
def calculate(ds: Dataset, settings: Settings, _pooled=None):
    ...
    for _,p in ds.products.iterrows():
        ...
        if cfg.forecast_method in ('adaptive','pooled'):
            ...
            fitted=forecast_series(corrected,future,method=cfg.forecast_method,seasonality=cfg.seasonality,trend=cfg.trend,supplied=ds.seasonal)
            scenario=(1+cfg.growth_percent/100)*(1+growth if has_source_growth else 1)
        ...
        if np.isfinite(stock):
            for day_index,d in enumerate(pd.date_range(asof,periods=sku_lead)):
                if not arriving.empty:balance+=float(arriving.loc[arriving.eta.eq(d),'quantity'].clip(lower=0).sum())
                balance-=daily_path[day_index]
                if balance<0:
                    shortage=True
        ...
        reason=(f'Спрос {forecast:.1f} + страховой запас {safety:.1f} − свободный остаток {stock:.1f} − поступления {due:.1f}. '
                f'Минимум {moq:g}, кратность {pack:g}. Исключено разовых продаж: {excluded:.1f}; восстановлено спроса: {lost:.1f}.') if np.isfinite(stock) else f'Введите актуальный свободный остаток. Порог пополнения: {max(0,forecast+safety-due):.1f} ед. Неизвестный остаток не подменён нулём.'
