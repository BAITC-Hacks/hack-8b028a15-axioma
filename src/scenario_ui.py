...
def render_scenarios(ds,cfg,result,histories,store,dataset_key,version,export_excel,labels):
    ...
    cols=['sku','name','recommended','unit','order_min','order_max','robustness','count_priority','count_reason']
    st.dataframe(selected[[c for c in cols if c in selected]].rename(columns=labels),hide_index=True,width='stretch')
